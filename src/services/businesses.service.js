...
class BusinessesService {
    ...
    async createBusiness(business) {
        ...
        if (!Array.isArray(products) || !products.length) {
            throw new HttpError('Products array is not valid', HTTP_STATUS.BAD_REQUEST);
        }
        ...
        if (!productsDB || !productsDB.length) {
            throw new HttpError('Products array is not valid. Please check the content', HTTP_STATUS.BAD_REQUEST);
        }
    ...
    async updateBusinessById(id, business) {
        ...
        const updatedBusiness = await businessesDAO.updateBusinessById(id, business);
    ...
    async addProductToBusiness(businessId, product) {
        ...
        const response = await businessesDAO.addProductToBusiness(businessId, product);
        return response;
    }
